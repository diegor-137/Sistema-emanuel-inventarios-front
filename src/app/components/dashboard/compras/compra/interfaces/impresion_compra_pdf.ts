...
export async function impresionCompra_pdf(factura:any){
        ...
        new Table([
         ...
         pdf.rawContent([
           new Columns([
               ...
               [
                   new Txt(`Numero factura proveedor: ${data.documento}`).alignment('center').end,
               ],
           ...
           new Columns([
               new Txt([new Txt("Generado en: ").bold().end,`${data.sucursal.nombre}\n${data.sucursal.direccion}`]).alignment('left').end,
               pdf.add(''),
           ]).columnGap(20).margin(5).end,
           new Columns([
               new Txt([new Txt("Realizado por: ").bold().end,new Txt(`${data.empleado.nombre} ${data.empleado.apellido}`).end]).end,
               '',
      ...
      pdf.add([
        ...
        new Table([
            [new Cell(new Txt('Total').bold().end).rowSpan(1).end,`Q. ${NumberFormatter(total, 2)}`],
   ...
   function extracData(data: any[]) {
    ...
    return data.map(row => [row.cantidad, row.nombre_p, row.precio, row.subtotal]);
   }