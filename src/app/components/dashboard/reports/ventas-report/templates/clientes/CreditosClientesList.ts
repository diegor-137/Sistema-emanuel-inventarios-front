...
export async function resultCreditosClientes(data:any){
 
        pdf.add(await createHeader(data.header, pdf));

        pdf.add([
            new Table([
                ['Fecha Credito', 'Cliente', '# Venta', 'Monto','Pagos', 'Saldo', 'Fecha Limite' ],
    ...
    return data.map(row => [new Date (row.fechainicio).toLocaleDateString("es-gt", {year: 'numeric', month: '2-digit', day: 'numeric', formatMatcher: "best fit"}),
                            row.cliente,row.compra,row.total,row.pagos,row.saldo,
